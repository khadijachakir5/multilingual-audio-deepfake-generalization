...
def write_reproducibility_files(df, output_dir):
    ...
    manifest_cols = [c for c in ["filepath", "language", "label",
                                  "generator", "split", "duration"]
                     if c in df.columns]
    df[manifest_cols].to_csv(out / "feature_manifest_ggmddc.csv", index=False)
    ...
    pd.DataFrame(rows).to_csv(out / "fold_definitions_ggmddc.csv", index=False)
...
def run():
    ...
    write_reproducibility_files(df_valid, OUTPUT_DIR)
