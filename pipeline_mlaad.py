...
def scan_datasets():
    ...
    n_real = sum(1 for it in items if it["label"] == 0)
    n_gens = len(set(it["model_name"] for it in items if it["label"] == 1))
    logger.info(f"  MLAAD v5 : {n_fake:,} fake | {n_gens} model_name uniques | "
    ...
    for it in fake_items:
        by_lang[it["language"]].add(it["model_name"])
...
def write_reproducibility_files(df, output_dir):

    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
    manifest_cols = [c for c in ["filepath", "language", "label",
                                  "generator", "model_name", "duration"]
                     if c in df.columns]
    df[manifest_cols].to_csv(out / "feature_manifest_mlaad.csv", index=False)

    rows = []
    for lang in sorted(df["language"].dropna().unique()):
        test = df["language"] == lang
        rows.append({"protocol": "LOLO", "held_out": lang,
                     "n_train": int((~test).sum()), "n_test": int(test.sum()),
                     "n_test_real": int(((test) & (df["label"] == 0)).sum()),
                     "n_test_fake": int(((test) & (df["label"] == 1)).sum())})
    real_n = int((df["label"] == 0).sum())
    for model in sorted(df.loc[df["label"] == 1, "model_name"].dropna().unique()):
        n_fake_test = int(((df["label"] == 1) & (df["model_name"] == model)).sum())
        n_real_test = min(n_fake_test, real_n)
        rows.append({"protocol": "LOGO", "held_out": model,
                     "n_train": int(len(df) - n_fake_test - n_real_test),
                     "n_test": int(n_fake_test + n_real_test),
                     "n_test_real": n_real_test, "n_test_fake": n_fake_test})
    pd.DataFrame(rows).to_csv(out / "fold_definitions_mlaad.csv", index=False)
...
def run():
    ...
    write_reproducibility_files(df, OUTPUT_DIR)
